<script lang="ts">
  ...
  if (schema.type !== 'array' || schema.items.type !== 'object') {
    throw new Error(
      'ArrayBlocks editor can only be used on an array with items of type=object',
    )
  }
  ...
  $: {
    const nonArrayProperties = Object.fromEntries(
      Object.entries(schema.items.properties).filter(
        ([propName, subschema]) =>
          (subschema as { type: string }).type !== 'array',
      ),
    )
    addItemSchema = {
      ...schema.items,
...
<div
  id={params.path.join('.')}
  class="subset array-blocks depth-{params.path.length}"
>
  <ol>
  ...
  {#if adding}
    ...
    <button
      type="button"
      class="submit-button new-item-submit"
      on:click={onAddUpdate}>Add</button
    >
  {/if}
</div>
