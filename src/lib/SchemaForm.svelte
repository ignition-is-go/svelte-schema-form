...
<script lang="ts">
  ...
  export let pathComponents: Record<string, new (...args: any[]) => any> = {}
  ...
  const pathChanged = (
    ...
    if (val === undefined && path.length > 0) {
      ...
      const parent = pathFront.length
        ? get(copyParamsVal, path.slice(0, -1))
        : params.value
    ...
    console.log(
      `dispatch value path: ${path.join('.')} val: ${JSON.stringify(val)},${op ? ' op: ' + op : ''} errors: ${JSON.stringify(validationErrors)}, succeeded: ${succeeded}`,
    )
